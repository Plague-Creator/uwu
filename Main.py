...
intro = (
    Fore.LIGHTCYAN_EX
    + """ 
    Wonderbot for an easy lobby bot! , http://bit.ly/38URYTD LOGIN WITH YOUR BOT ACCOUNT HERE COPY AND PASTE THE CODE! 
...
with open("config.json") as f:
    try:
        ...
        print(
            ...
            + " [ERROR] "
            + Fore.RESET
            + "There was an error in one of the bot's files! (config.json). WonderBot for an easy lobby bot! "
        )
...
with open("info.json") as f:
    try:
        ...
        print(
            ...
            + " [ERROR] "
            + Fore.RESET
            + "There was an error in one of the bot's files! (info.json) WonderBot for an easy lobby bot! "
        )
        print(Fore.LIGHTRED_EX + f"\n {e}")
        exit(1)
...
async def event_ready():
    ...
    await member.edit_and_keep(
        ...
        partial(
            ...
            icon='InfluencerBanner27',
            color='DefaultColor35',
            season_level='1500',
        ),
        partial(
...
async def event_party_member_join(member: fortnitepy.PartyMember) -> None:
    ...
    await client.party.send(
        f" Salve {member.display_name}, scrivimi !help per una lista di comandi \n to get your bot Youtube: WonderBot \n  tiktok. @SimonLeaks!"
    )
...
@client.command()
async def wonderbot(ctx):
    await ctx.send('con wonderBot puoi creare il tuo bot in meno di 2 minuti!')
...
async def pinkghoul(ctx):
    ...
    await ctx.send('ciao! sono un bot creato da Anonimo-1975...Per ottenere il tuo bot: Per ottenere il tuo bot: \n1)YouTube: WonderBot\n2) TikTok: Simon_leaks')
@client.command()
...
async def magic(ctx: fortnitepy.ext.commands.Context):
    ...
    await ctx.send('hai visto? sono scomparso')   
...
async def defaults(ctx):
    await ctx.send(f"vecchi defaults..")
    ...
    await ctx.send(
        f"ecco tutti i defaults! "
    )
    await ctx.send('ciao! sono un bot creato da Anonimo-1975...Per ottenere il tuo bot: Per ottenere il tuo bot: \n1)YouTube: WonderBot\n2) TikTok: Simon_leaks')
@client.command()
async def exclusive(ctx):
      await ctx.send(f"skin esclusive..")
      ...
      await asyncio.sleep(7)
      await ctx.send(f"ciao! sono un bot creato da Anonimo-1975...Per ottenere il tuo bot: Per ottenere il tuo bot: \n1)YouTube: WonderBot\n2) TikTok: Simon_leaks")
...
async def s4(ctx):
      ...
      await asyncio.sleep(5)
      await ctx.send(f"Per ottenere il tuo bot: \n2)YouTube: WonderBot\n3) TikTok: Simon_leaks")
      await client.party.me.set_emote(asset="EID_Dab")
      await ctx.send(
        f"S4 reel"
      )
...
async def anime(ctx):
    ...
    await ctx.send('non mi piacciono gli anime ma se proprio lo vuoi...')    
...
async def gift(ctx):
    ...
    await ctx.send(f"che cosa cabbo pensavi succedesse? hahah vuoi qualcosa BENE compratela")
...
async def og(ctx):
    ...
    await ctx.send(
      f"ciao! sono un bot creato da Anonimo-1975...Per ottenere il tuo bot: Per ottenere il tuo bot: \n1)YouTube: WonderBot\n2) TikTok: Simon_leaks"
     )

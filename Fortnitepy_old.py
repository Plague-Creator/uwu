    ...
    try:
        ...
        if extension in [".py", ".bat", ".txt", ".md", ".sh", ""]:
            ...
            if current.replace('\n','').replace('\r','').encode(encoding='utf-8') != github.decode().replace('\n','').replace('\r','').encode(encoding='utf-8'):
                ...
                if os.path.isfile(f'{filename_}_new{extension}'):
                    try:
                        os.remove(f'{filename_}_new{extension}')
                    except PermissionError:
                        print(f'Failed to remove file {filename}\n')
                        print(traceback.format_exc())
                try:
                    os.rename(filename, f'{filename_}_new{extension}')
                except PermissionError:
                ...
                try:
                    if os.path.isfile(f'{filename_}_new{extension}'):
                        try:
                            ...
                            print(f'Failed to remove file {filename_}_new{extension}')
...
if "-beta" in sys.argv:
    githuburl = "https://raw.githubusercontent.com/Ghost-ikon/GhostFN/beta/"
else:
    githuburl = "https://raw.githubusercontent.com/Ghost-ikon/GhostFN/master/"
